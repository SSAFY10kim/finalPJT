...
- PCA는 변인을 탐색해서 변환을 통해 주선분을 결정하는 방법이다. target을 분류할 때 몇개의 변수로 분류할 수 있는지 결론적으로 PCA의 본질은 차원 축소이다. 차원이 축소됐다는 것은 원본 데이터가 아니라 변환(projection) 된 데이터 == 주성분을 이용해 분석 혹은 모델링을 진행하겠다는 것이다. 
- K-means 클러스터링 k 결정
